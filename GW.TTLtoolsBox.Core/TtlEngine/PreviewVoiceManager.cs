...
namespace GW.TTLtoolsBox.Core.TtlEngine
{
    ...
    public class PreviewVoiceManager : IDisposable
    {
        ...
        /// <param name="speed">语速值，默认100</param>
        /// <param name="volume">音量值，默认100</param>
        /// <returns>预览声音文件路径</returns>
        public string GetPreviewFilePath(string engineId, string sourceName, int speed = 100, int volume = 100)
        {
            ...
            return Path.Combine(_previewFolder, $"{engineId}_{safeSourceName}_{speed}_{volume}.mp3");
        }
        ...
        public string GetStatusFilePath(string engineId, string sourceName, int speed = 100, int volume = 100)
        {
            if (string.IsNullOrWhiteSpace(engineId) || string.IsNullOrWhiteSpace(sourceName))
            {
                return null;
            ...
            string safeSourceName = getSafeFileName(sourceName);
            return Path.Combine(_previewFolder, $"{engineId}_{safeSourceName}_{speed}_{volume}.generating");
        }
        ...
        public PreviewVoiceStatus GetStatus(string engineId, string sourceName, int speed = 100, int volume = 100)
        {
            if (string.IsNullOrWhiteSpace(engineId) || string.IsNullOrWhiteSpace(sourceName))
            {
            ...
            string cacheKey = $"{engineId}_{sourceName}_{speed}_{volume}";
            ...
            PreviewVoiceStatus status = calculateStatus(engineId, sourceName, speed, volume);
        ...
        public PreviewVoiceStatus RefreshStatus(string engineId, string sourceName, int speed = 100, int volume = 100)
        {
            if (string.IsNullOrWhiteSpace(engineId) || string.IsNullOrWhiteSpace(sourceName))
            {
                return PreviewVoiceStatus.未生成;
        ...
        /// <param name="speed">语速值，默认100</param>
        /// <param name="volume">音量值，默认100</param>
        public void SetStatus(string engineId, string sourceName, PreviewVoiceStatus status, int speed = 100, int volume = 100)
        {
            if (string.IsNullOrWhiteSpace(engineId) || string.IsNullOrWhiteSpace(sourceName))
            {
                return;
            ...
            string statusFilePath = GetStatusFilePath(engineId, sourceName, speed, volume);
            string previewFilePath = GetPreviewFilePath(engineId, sourceName, speed, volume);
        ...
        public PreviewVoiceTask CreateTask(string engineId, string sourceName, int speed = 100, int volume = 100)
        {
            if (string.IsNullOrWhiteSpace(engineId) || string.IsNullOrWhiteSpace(sourceName))
            {
                return null;
            ...
            PreviewVoiceTask task = new PreviewVoiceTask
            {
                ...
                Volume = volume,
                Text = GetPreviewText(),
                OutputFilePath = GetPreviewFilePath(engineId, sourceName, speed, volume),
        ...
        /// <param name="speed">语速值，默认100</param>
        /// <param name="volume">音量值，默认100</param>
        public void ResetStatus(string engineId, string sourceName, int speed = 100, int volume = 100)
        {
            string statusFilePath = GetStatusFilePath(engineId, sourceName, speed, volume);
            if (!string.IsNullOrWhiteSpace(statusFilePath) && File.Exists(statusFilePath))
            {
            ...
            RefreshStatus(engineId, sourceName, speed, volume);
        }
        ...
        /// <param name="volume">音量值，默认100</param>
        /// <returns>预览声音状态</returns>
        private PreviewVoiceStatus calculateStatus(string engineId, string sourceName, int speed = 100, int volume = 100)
        {
            string previewFilePath = GetPreviewFilePath(engineId, sourceName, speed, volume);
            string statusFilePath = GetStatusFilePath(engineId, sourceName, speed, volume);
